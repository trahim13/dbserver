...
public final class GenericIndex {
    ...
    //String name of the index, String value,  rowNUmber
    ...
    public long getBytePosition(long rowNumber) {
        return GenericIndex.getInstance().rowIndex.getOrDefault(rowNumber, -1L);
    }
    ...
    private synchronized void remove(long row) {
        ...
        if (nameToDelete != null) {
            this.indexes.remove(nameToDelete);
        }
    }
    ...
    public void addNameToIndex(final String indexedValue, long rowIndex) {

        if (!this.indexes.containsKey(this.schema.indexBy)) {
            this.indexes.put(this.schema.indexBy, new ConcurrentHashMap<>());
        }
